...
public class RoomService
{
				...
				Long userId)
	{
		...
		if (userId != null)
		{
			List<UserQuestionProgress> progressList = progressRepo.findByUserIdAndThemeId(userId,
						(long) roomId);
	 ...
	 */
	private AnswerResultDto evaluateGap(Question question, AnswerRequest request)
	{
	 ...
	 */
	private AnswerResultDto evaluateMcTf(Question question, AnswerRequest request)
	{
	...
	private Theme getTheme(int roomId)
	{
		...
		return themeRepo.findById((long) roomId).orElseThrow(() -> new NoSuchElementException(
	...
	private List<Question> loadAllQuestionsForTheme(int roomId)
	{
		...
		List<Question> withAnswers = questionRepo.findByThemeIdWithAnswers(themeId);

		// 2. Alle Fragen mit GapFields+Options (für GAP)
		List<Question> withGaps = questionRepo.findByThemeIdWithGaps(themeId);
				...
				List<Long> selectedAnswerIds)
	{
		...
		UserQuestionProgress progress = progressRepo.findById(id).orElseGet(() ->
		{
	 ...
	 */
	private QuestionDto toQuestionDto(Question q, int currentIndex, int total)
	{
